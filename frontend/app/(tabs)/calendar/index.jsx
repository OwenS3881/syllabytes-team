...
import { SafeAreaView, ScrollView, FlatList, Dimensions, View, Text, StyleSheet, Image, TouchableOpacity } from "react-native";
import {addWeeks, addDays, startOfWeek, isSameDay, format, differenceInMinutes,} from 'date-fns';
...
function WeekGrid({ weekStart, events }) {
  ...
  return (
    <View style={[styles.gridContainer, { height: gridHeight, width: SCREEN_WIDTH }]}>
      {/* month & year label header */}
      <View style={styles.MonthHeader}>
        <Text style={styles.MonthHeaderText}>{format(weekStart, 'MMMM yyyy')}</Text>
      </View>
      ...
      <View style={styles.DayHeaderRow}>
        ...
        {days.map((day) => (
          <View key={day.toISOString()} style={[styles.DayHeaderCell, { width: DAY_COLUMN_WIDTH }]}>
            <Text style={styles.DayHeaderText}>{format(day, 'EEE')}</Text>
            <Text style={styles.DayHeaderNumber}>{format(day, 'd')}</Text>
          </View>
        ))}
      </View>

      {/* scrollable times + day columns */}
      <ScrollView style={{ flex: 1 }} contentContainerStyle={{ height: gridHeight }}>
        <View style={{ flexDirection: 'row', flex: 1 }}>
          {/* Time Labels Column */}
          <View style={styles.timeLabelsCell}>
            {hours.map((hour) => (
              <View key={hour} style={[styles.timeLabelCell, { height: HOUR_HEIGHT }]}>
                <Text style={styles.timeLabelText}>{format(new Date().setHours(hour, 0, 0, 0), 'ha')}</Text>
              </View>
            ))}
          </View>
          ...
          {/* Days Columns */}
          {days.map((day) => (
            <View key={day.toISOString()} style={{ width: DAY_COLUMN_WIDTH, borderLeftColor: Colors.borderGray, borderLeftWidth: 1 }}>
              {/* Horizontal hour lines */}
              {hours.map((h) => (
                <View key={h} style={{ height: HOUR_HEIGHT, borderTopColor: Colors.borderGray, borderTopWidth: 1 }} />
              ))}
...
const renderWeek = ({ item }) => (
  ...
  const goPrevWeek = () =>
    listRef.current?.scrollToIndex({ index: pivot + (currentOffset - 1), animated: true });
  const goNextWeek = () =>
    listRef.current?.scrollToIndex({ index: pivot + (currentOffset + 1), animated: true });
  const goToday = () =>
  ...
  return (
    ...
    <SafeAreaView style={styles.basepage}>
       <View style={styles.navBar}>
        ...
        <Text style={styles.navMonth}>{format(visibleWeekStart, "MMMM yyyy")}</Text>
      </View>
